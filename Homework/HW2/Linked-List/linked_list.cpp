// Name: Aidan Cowan
// Date: 9/6/23
// Program: ECGR 3180 - Homework 2: Linked List
// Description:
// 
// Concatenate two input singly linked lists l1 and l2 
// into a new output linked list that contains all the 
// nodes of l1 followed by all the nodes of l2.
...
void concat(SLinkedList<T>& l1, SLinkedList<T>& l2, SLinkedList<T>& lout) {
	...
	while (current != nullptr) { // Iterate through l1, add l1 elements to backend of temp list
		lout.addBack(current->elem);
		current = current->next;
	}

	current = l2.head;
	while (current != nullptr) { // Iterate through l2, add l2 elements to backend of temp list
		lout.addBack(current->elem);
		current = current->next;
	}
}
